...
| 2026-09-03 | 0.2 | Reorganización a `server/ web/ mobile/ deploy/ docs/` con `git mv` (rama `fase0/reorganizar`, commit `253c35d`). `README.md` nuevo. Sin cambios de contenido en código. | `git status` limpio · 129 renombrados, 0 borrados, 0 modificados · `git log --follow` conserva historial · raíz solo con README, CLAUDE, AUTHORS, .gitignore, .env.example y carpetas. | 253c35d (rama fase0/reorganizar) |

## Próximo paso

Integrar `fase0/reorganizar` en `main` (PR). Después: **0.3 — docs/DECISIONES.md** (2 h).
